...
def set_up_fit_extinction(p,p_fixfree,model_name,extinction_model,mixed_or_screen):
    ...
    if mixed_or_screen == 'S':
        exp = "10**(-0.4*%s_Av*log10(%s))" % (model_name,extinction_model)
...
def set_up_fit_model_scale(p,p_fixfree,name):
    ...
    model_name = model_name
    exp = "%_amp*%s" % (model_name,model_name)
    model_scale_model = ExpressionModel(exp,independent_vars=[model_name])
    model_scale_parameters = model_scale.make_params()
    
    model_scale_parameters[model_name+'_amp'].set(value=p[0],min=0,vary=p_fixfree[0])
...
def set_up_absorption(p,p_fixfree,model_name,abs_model):
    ...
    exp = "exp(%s_tau*-1*%s)" % (model_name,abs_model)
    model = ExpressionModel(exp,independent_vars=[abs_model],name = model_name)
    abs_model_parameters = model.make_params()
    abs_model_parameters[model_name+'_tau'].set(value=p[0],min=0,vary=p_fixfree[0])
