...
def Spitzer_IRAS21219m1757():
    ...
    for i in lines:
        ...
        zinit_gas[i][2,18,:]=MIRz
        zinit_gas[i][4,21:22,:]=MIRz
        zinit_gas[i][5,21,:]=MIRz
        zinit_gas[i][6:8,23:25,:]=MIRz
        zinit_gas[i][1:4,1:7,:]=MIRz
        zinit_gas[i][6:16,0:7,:]=MIRz
        zinit_gas[i][11,8:9,:]=MIRz
        zinit_stars=np.full((ncols,nrows),MIRz)
    ...
    init = { \
            ...
            #'fcninitpar': 'questfit',
            'fcncontfit': 'questfit',
            'fitran': [ float(config_file['source'][1])*1e4, float(config_file['source'][2])*1e4 ],	# angstrom
            #'fluxunits': 1e-15,  # erg/s/cm^2/arcsec^2
            'infile': directory+config_file['source'][0].replace('.ideos','').split('.npy')[0]+'_mock_cube.fits',
            ...
            'zerodq': True,
            'noemlinfit': b'1',
            'argscontfit': {'config_file': MIRcffile, 'global_ice_model': global_ice_model, 'global_ext_model': global_ext_model},
