...
;      2020may26, YI, tranlated to Python 3
;    
; :Copyright:
...
def kcwi(linelist, linelistz,linetie,initflux,initsig,maxncomp,ncomp,
...
# cycle through velocity components
    ...
    # cycle through lines
        ...
        linea = np.where(lines_arr == '[NeV]3345')[0]
        cta = len(linea)
        lineb = np.where(lines_arr == '[NeV]3426')[0]
        ctb = len(lineb)
        if cta > 0 and ctb > 0 :
            parinfo[foff+linea*3]['tied'] = 'P['+'{:1}'.format(str(foff+lineb*3))+']/2.8'
            parinfo[foff+linea*3]['flux_tie'] = '[NeV]3426'
            # Make sure initial value is correct
            parinfo[foff+linea*3]['value'] = parinfo[foff+lineb*3]['value']/2.8
