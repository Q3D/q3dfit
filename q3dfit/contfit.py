...
def questfit(wlambda, flux, weights,
             singletemplatelambda, singletemplateflux,
    ...
    flux: array
        Flux values to be fit. Assumed units are Jy; will be transformed to
        1e-10 erg/s/cm2/mu/sr below.
    ...
    # holds PAH template, blackbody, silicate emission, and powerlaw components
    emcomps = dict()
    # holds all components, including extinction, absorption components
    allcomps = dict()
    ...
    config_file = questfitfcn.readcf(config_file)
    ...
    for i in config_file.keys():
        ...
        if 'blackbody' in i:
            # initiaal parameters and name for this component
        ...
        if 'powerlaw' in i:
            ...
            extinction_model = config_file[i][3]
            ice_model = config_file[i][9]
            # Initialize component. Set scale factor to 1, power law
    ...
    for i in allcomps.keys():
        # Look for it in the right directory
...
if do_test:
    ...
    # config_file = questfitfcn.readcf(filename)
    ...
    singletemplatelambda = None
    singletemplateflux = None
    index = None
