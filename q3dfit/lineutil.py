...
def jwstlinez(z: float,
              ...
              outdir: Optional[str]=None) -> Table:
    '''
    Creates a table of emission lines expected to be found in a given
    instrument configuration for JWST. The 'observed' column displays a 
    redshifted emission line center calculated using the input z value. 
    Therefore, it is only as precise as the known z value.
    ...
    spelling. Otherwise, the function will return errors.

    Parameters
    ----------
    z
        Galaxy, systemic redshift. Used for redshifting emission lines
        for 'observed' column.
    gal
        Galaxy name for filenaming.
        Can also be customized for any other desired filename designations
    instrument
        JWST instrument. Inputs: NIRSpec, MIRI
    ...
    mode
        ...
        MIRI Inputs: MRS
    grat_filt
        Grating and filter combination for NIRSpec or channel for MIRI.
        NIRSpec Inputs:
        ...
        Ch4_A, Ch4_B, Ch4_C
    waveunit
        Optional. Determines the unit of input wavelengths and output table.
        Inputs are 'micron' or 'Angstrom'. Default is 'micron'.
    outdir
        Optional. Output directory. Default is None, meaning no output file is saved.
    ...
    Table
        ...
        Internally, everything is processed in microns, so filename inclues
...
def observedlinez(z: float,
    ...
    lamb_max
        ...
        value
    vacuum
        Optional. If false, enables conversion to air wavelengths. 
        Default is True, meaning all wavelengths are in vacuum.
    waveunit
        Optional. Determines the unit of input wavelengths and output table.
        Inputs are 'micron' or 'Angstrom'. Default is 'micron'.
    outdir
