...
def xdr_templ_to_npy(file_xdr='draine03.xdr', path_out=os.path.normpath('../data/questfit_templates/')):
    '''
...
In: xdr template file, f.ex. draine03.xdr
    ...
    templ_name = os.path.splitext(os.path.basename(file_xdr))[0]
    np.save(os.path.join(path_out, templ_name)+'.npy', data)
    np.savetxt(os.path.join(path_out, templ_name)+'.meta', np.array([header]), fmt='%s')
...
def example_sources_to_npy():
    ...
    and only the 'WAVE', 'FLUX', 'STDEV' and 'FLAG' entries for IRAS21219m1757_dlw_qst.xdr.
    '''
    dir_in=os.path.normpath('../test/test_questfit/')
    path_out=dir_in

    dict1 = readsav(os.path.join(dir_in, '4978688_0.ideos.xdr'), python_dict=True, verbose=False)
    dict2 = readsav(os.path.join(dir_in, 'IRAS21219m1757_dlw_qst.xdr'), python_dict=True, verbose=False)
    ...
    np.save(os.path.join(path_out, '4978688_0.ideos.npy'), data1.view(np.recarray))

    data2 = dict2['memstor']['AAR'][0]['DATA']
    np.save(os.path.join(path_out, 'IRAS21219m1757_dlw_qst.npy'), data2)
...
parser.add_argument('--path_tpl', default=os.path.normpath('../data/questfit_templates/'), help='Path pointing to the input .xdr templates')
parser.add_argument('--path_out', default=os.path.normpath('../data/questfit_templates/'), help='Path where the output .npy templates should be stored')
...
path_si = os.path.join(path_tpl, 'silicatemodels/')

if not os.path.isdir(path_out):	os.mkdir(path_out)
if not os.path.isdir(os.path.join(path_out, 'silicatemodels/')):	os.mkdir(os.path.join(path_out, 'silicatemodels/'))
...
for file_i in glob.glob(os.path.join(path_tpl, '*.xdr')):
	xdr_templ_to_npy(file_i, path_out=path_out)

for file_i in glob.glob(os.path.join(path_si, '*.xdr')):
	xdr_templ_to_npy(file_i, path_out=os.path.join(path_out, 'silicatemodels/'))
